<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex items-center justify-center px-4 sm:px-6 lg:px-8 py-12">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-600 dark:text-slate-400 mt-2">Entre para criar e compartilhar presentes</p>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 sm:p-10 border border-slate-200 dark:border-slate-700">
        <div class="mb-8">
          ...
          <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Entre na sua conta para continuar</p>
        </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
          <p class="text-center text-sm text-slate-600 dark:text-slate-400">
            Não tem uma conta?
            <router-link to="/register" class="font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
    ...
    async submit() {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('auth-changed', { 
          detail: { user: res.data.user, action: 'login' } 
        }))
        ...
        if (e.response?.status === 401) {
          this.error = 'E-mail ou senha incorretos. Verifique suas credenciais e tente novamente.'
