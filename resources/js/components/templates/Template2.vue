<template>
  <div class="min-h-screen bg-gradient-to-br from-pink-400 via-rose-300 to-orange-300 p-3 sm:p-4 md:p-6 lg:p-8 relative overflow-hidden">
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <!-- Polaroid Gallery -->
      <div v-if="visualMedia.length" class="mb-8 md:mb-12">
            ...
            <div class="polaroid-photo">
              <transition name="fade" mode="out-in">
                ...
                <video 
                  ...
                  :src="`/storage/${currentMedia.path}`" 
                  controls 
                  class="w-full h-72 sm:h-96 object-contain bg-black"
                />
              </transition>
            </div>
            <div class="polaroid-caption">
              <p class="text-gray-600 text-lg" style="font-family: 'Comic Sans MS', cursive;">Memória {{ index + 1 }} de {{ visualMedia.length }}</p>
            </div>
          </div>
        ...
        <div v-if="visualMedia.length > 1" class="flex justify-center gap-2 mt-4">
          <button
            ...
            :class="index === i ? 'bg-rose-500 scale-110' : 'bg-white/60'"
          />
        </div>
      </div>
      ...
      <div v-if="audioMedia.length" class="space-y-4 mb-6 md:mb-8">
        <div 
          ...
          :key="m.id"
          class="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-xl"
        >
          <div class="text-center mb-4 text-4xl">🎵</div>
